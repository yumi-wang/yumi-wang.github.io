...
document.getElementById(
  'para2-5'
).innerHTML = `The following example will explain how a loop works. Inside the bracket after the while key work, is the condition. If the student_score doesn't meet the condition, the while loop will go on again until student_score is equal to or more than 50.`
...
document.getElementById(
  'para3-6'
).innerHTML = `If we go further into the command lines, the <span id="text-highlight">document</span> represents the Document Object. When you load an HTML document in a browser, it is converted into a document object. We can access the document object by typing document into the Console-tab as we did before. `
document.getElementById(
  'para3-7'
).innerHTML = `The Document method <span id="text-highlight">querySelector('p')</span> returns the first Element within the HTML document that matches the specified selector, or group of selectors. As we only have one paragraph sector in the document, this paragraph will be returned.`
document.getElementById(
  'para3-8'
).innerHTML = `The <span id="text-highlight">innerHTML</span> is a property of the Element that allows you to get or set the HTML markup contained within the element. It can simply be understood as "changing the text in the selected sector". As we have already locked in the paragraph within the HTML document. To manipulate the content, we only need to assign the new text with <span id="text-highlight">= "This is how we manipulate DOM!"</span> `
document.getElementById(
  'para3-9'
).innerHTML = `If you want to try it out, changing the background color of the page. Open the developer tools (F12) and in the console, type in: 
...
document.getElementById(
  'para4-2'
).innerHTML = `The fruits are stored in a numbered order, and the index starts from 0. So if we want to call out coconut, we use fruits[0] to get it. Because the index of the first item is 0, and the index is wrapped in the square brackets [].`
document.getElementById(
  'para4-3'
).innerHTML = `An object is another data type in JavaScript. It is used to store keyed collections of various data and more complex entities. The collections are stored within figure brackets {}, and comma "," is used to separate each collection. They are like business cards. We can store different categories of data in each object. For instance:`
document.getElementById(
  'para4-4'
).innerHTML = `How about accessing properties on the Object? We can access the property values by their property names/keys. There two ways: dot notation and bracket notation:`
document.getElementById(
  'para4-5'
).innerHTML = `Remember to use quotation marks in the brackets when you are using the bracket notation. Otherwise, you will get an undefined error. However, if you assign a property name to a new variable, quotation marks are not needed as we are accessing the value via a variable name instead of a string. `
document.getElementById(
  'para4-6'
).innerHTML = `Another thing to note is if the property names are two separate words, we can only use bracket notation to access the value.`
...
document.getElementById(
  'para5-2'
).innerHTML = `The example shows how function is defined and called.`
