...
document.getElementById(
  'para2-5'
).innerHTML = `The following example will explain how loop works. Inside the bracket after the while key work, is the condition. If the student_score doesn't meet the condition, the while loop will go on again until student_score is equal to or more than 50.`
...
document.getElementById('para3-3').innerHTML =
  'From the perspective of DOM, it sees web documents this way'
...
document.getElementById('para3-8').innerHTML = 
  `The <span id="text-highlight">innerHTML</span> is a property of the Element that allows you to get or set the HTML markup contained within the element. It can simply be understood as "changing the text in the selected sector". As we have already locked in the paragraph within the HTML document. To manipulate the content, we only need to assign the new text with <span id="text-highlight">= "This is how we manipulate DOM!"</span> `
document.getElementById('para3-9').innerHTML = 
  `If you want to try it out, changing the background color of the page. Open the developer tools and in the console, type in: 
